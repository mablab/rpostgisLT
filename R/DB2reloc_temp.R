#' Insert relocations from a source table into the table 'reloc_temp'. If 
#' relocations are given as X,Y coordinates, they are converted into a POINT 
#' geometry in PostGIS.
#' 
#' It assumes valid input, thus pre-condition 
#' 
...
#' 
#' @examples 
#' DB2reloc_temp(conn, schema = "traj_t1", 
...
#' 
###############################################################################
DB2reloc_temp <- function(conn, schema, relocation_data, pgtrajs, animals,
        bursts=NA, relocations, timestamps, rids, srid) {
    ...
    # Begin transaction block (hence make_reloc_temp() operation is "all or nothing")
    ...
    if (pgtrajs %in% fields) {
        ...
        query <- paste0("UPDATE reloc_temp
        ...
        # Use the string
        query <- paste0("UPDATE reloc_temp SET p_name = '", pgtrajs, "';")
        invisible(dbGetQuery(conn, query))
    }
    ...
    if (animals %in% fields) {
        ...
        # Use the string
        query <- paste("UPDATE reloc_temp SET a_name = '", animals, "';")
        invisible(dbGetQuery(conn, query))
    }
    ...
    if (bursts %in% fields) {
        ...
        query <- paste0("UPDATE reloc_temp SET b_name = '",animals,"';")
        invisible(dbGetQuery(conn, query))
    } else {
        # Use the string
