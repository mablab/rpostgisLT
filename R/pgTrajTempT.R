...
pgTrajTempT <- function(conn, schema) {
    ...
    if ('qqbqahfsbrpq' %in% tables$tablename) {
        acr <- NA
        while(is.na(acr) | !(acr %in% "y" | acr %in% "n")) {
            acr <- readline(paste("A table named 'qqbqahfsbrpq' already exists in the schema", schema,
        ...
        if (acr %in% "n") {
            ...
            query <- paste0("DROP TABLE IF EXISTS ", schema, ".qqbqahfsbrpq;")
            invisible(dbGetQuery(conn, query))
        }
    }
    
    # Create 'qqbqahfsbrpq' table
    query <- paste0("CREATE TABLE ", schema, ".qqbqahfsbrpq (
                    id               serial,
                    pkey             text,
                    geom             geometry,
