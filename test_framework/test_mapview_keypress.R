...
# understands by default. Coordinate transformation in R can be expensive.
source("./makeShinyView.R")
...
pw <- # password
drv <- "PostgreSQL"
cs(pw=pw, drv = drv, host = "local")

# Data for PoC ------------------------------------------------------------

# load Ibex data into DB
data(ibex)
attr(ibex, "proj4string") <- CRS("+init=epsg:27572") # set CRS
