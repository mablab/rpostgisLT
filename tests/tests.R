...
print(system.time({
  ...
  ltraj2pgtraj(conn, ltraj = ib_min_srs, schema = "traj_min", pgtraj = "ib_min_3395")
  ...
  dbColumn(conn, c("traj","infolocs_ibex"), colname = "test",coltype = "text")
  ...
  pgInsert(conn, name = c("example_data", "ibex"), data.obj = ibex_dl, df.mode = TRUE) ## df mode converts times correctly
  ...
  as_pgtraj(conn, 
          ...
          relocations = c("x", "y"),
          timestamps = "date",
          rids = ".db_pkid",  # this is df.mode primary key default name
          srid = 3395)
  ...
  pgInsert(conn, name = c("example_data", "albatross"), data.obj = albatross_dl, df.mode = TRUE) ## df mode converts times correctly
